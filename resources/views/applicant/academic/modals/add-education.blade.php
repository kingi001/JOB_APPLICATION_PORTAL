<flux:modal name="add-education" class="max-w-4xl" variant="default" :show="$errors->any() || session('openModal') === 'add-education'">
    <form method="POST" action="{{ route('education.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf

        <div>
            <flux:heading size="lg">Add Academic Qualifications</flux:heading>
            <flux:text class="mt-2">Please provide your educational information, starting with the most recent.</flux:text>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <flux:input size=sm label="Institution" name="institution" value="{{ old('institution') }}" placeholder="e.g. University of Nairobi" required autofocus />
            ...
            <flux:input label="Course" name="field_of_study" value="{{ old('field_of_study') }}" placeholder="e.g. Computer Science" required />
            ...
            <flux:input size=sm label="Start Date" name="start_date" type="date" value="{{ old('start_date') }}" required />
            <flux:input size=sm label="End Date" name="end_date" type="date" value="{{ old('end_date') }}" required />
        </div>
